...
function onLoginSuccess() {
    requireFreshAuth({ pageTag: 'value', onAuthorized: fetchInventoryValueData, activeKey: 'value' });
}

initProtectedPage('value', fetchInventoryValueData, 'value');
...
function formatWanUnit(value, unit) {
...
function formatWanAmount(value) {
    return formatWanUnit(value, '万RMB');
}

function formatWanQuantity(value) {
    return formatWanUnit(value, '万pcs');
}
...
function calculateAndRenderCards(records) {
    ...
    let huianAmount = 0;
    let huianQuantity = 0;
    let quanzhouAmount = 0;
    let quanzhouQuantity = 0;
    ...
    records.forEach(function(record) {
        ...
        huianAmount += toNumber(getField(f, '惠安库存金额'));
        huianQuantity += toNumber(getField(f, '惠安库存数量'));
        quanzhouAmount += toNumber(getField(f, '泉州库存金额'));
        quanzhouQuantity += toNumber(getField(f, '泉州库存数量'));
    });

    // 业务口径：金额与库存成对展示，减少同一口径被拆成两张卡片造成的阅读成本。
    document.getElementById('card-total-amount').innerText = formatWanAmount(overseasAmount + domesticAmount);
    document.getElementById('card-total-quantity').innerText = formatWanQuantity(overseasQuantity + domesticQuantity);
    document.getElementById('card-overseas-amount').innerText = formatWanAmount(overseasAmount);
    document.getElementById('card-overseas-quantity').innerText = formatWanQuantity(overseasQuantity);
    document.getElementById('card-supplier-amount').innerText = formatWanAmount(domesticAmount);
    document.getElementById('card-supplier-quantity').innerText = formatWanQuantity(domesticQuantity);
    document.getElementById('card-huian-amount').innerText = formatWanAmount(huianAmount);
    document.getElementById('card-huian-quantity').innerText = formatWanQuantity(huianQuantity);
    document.getElementById('card-quanzhou-amount').innerText = formatWanAmount(quanzhouAmount);
    document.getElementById('card-quanzhou-quantity').innerText = formatWanQuantity(quanzhouQuantity);
    document.getElementById('card-jiege-amount').innerText = formatWanAmount(jiegeAmount);
    document.getElementById('card-jiege-quantity').innerText = formatWanQuantity(jiegeQuantity);
}
